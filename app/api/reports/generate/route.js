...
async function runGenerate({ market, period_start, period_end }, createdBy) {
  ...
  for (let i = 0; i < vendorData.length; i++) {
    ...
    const proactiveRows = buildProactiveReporting(vendor, allTasks, aiSections?.complaint_indices ?? null, lifetimeMaintByProperty, aiSections?.complaint_excerpts ?? {}, aiSections?.complaint_tiers ?? {});
