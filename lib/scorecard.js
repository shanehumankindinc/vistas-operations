...
export function buildTaskReviewMap(tasks, reviews, bridge) {
  ...
  const sortedTasks = [...tasks].filter(isCleanTask).filter(t => t.scheduled_date).sort((a, b) =>
  ...
  for (const task of sortedTasks) {
    ...
    // Both submitted_at and scheduled_date are date-only columns — they always parse
    // to UTC midnight, so subtraction gives exact integer calendar days. No anchor needed.
    //
    // >= 1 (not >= 0): a same-day review (submitted_at = scheduled_date) is from the
    // OUTGOING guest whose stay was prepared by an EARLIER clean, not the same-day
    // turnover clean. 95%+ of same-day reviews have a prior clean within 60 days that
    // correctly claims them. Using >= 0 would attribute those reviews to the wrong cleaner.
    const taskDateMs = new Date(taskDate + "T00:00:00Z").getTime();
...
// Annotate each clean task with on_time, deadline, matched review, linked refunds, and linked issues.
