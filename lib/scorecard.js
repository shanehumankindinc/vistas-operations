...
export function buildTaskReviewMap(tasks, reviews, checkIns, bridge) {
  ...
  // Build confirmation_code → check_out_date map from checkins
  const codeToCheckout = {};
  for (const ci of checkIns || []) {
    if (ci.confirmation_code && ci.check_out_date) {
      codeToCheckout[ci.confirmation_code] = ci.check_out_date;
    }
  }
  ...
  // Pass 1: exact match via confirmation_code → checkout date → task
  for (const review of reviews) {
    if (!review.confirmation_code) continue;
    const checkoutDate = codeToCheckout[review.confirmation_code];
