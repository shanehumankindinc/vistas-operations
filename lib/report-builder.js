...
function extractComplaintExcerpt(review) {
  ...
  if (chosen.length < 4) return null; // trivially short (e.g. a lone period)
...
export function buildProactiveReporting(vendor, allTasks, aiComplaintIndices = null, lifetimeMaintByProperty = {}) {
  ...
  for (const task of complaintTasks) {
    ...
    // lifetime_vendor_tasks: how many maintenance tasks this vendor's team has EVER filed
    // at this property (from the bulk historical query). null = not queried (non-complaint path).
    const lifetime_vendor_tasks = task.bz_property_id != null
      ? (lifetimeMaintByProperty[task.bz_property_id] ?? null)
      : null;
    const is_chronic_miss = !filedTask && lifetime_vendor_tasks === 0 && cleans_this_period >= 3;
...
function renderProactiveReporting(proactiveRows) {
  ...
  const tableRows = proactiveRows
    .map((r) => {
      ...
      let rowBg;
      if (filed) {
        statusCell = `<td style="padding:8px 10px;color:#065f46;font-weight:600;white-space:nowrap;">&#10003; Filed</td>`;
        rowBg = "#f0fdf4";
      } else if (r.is_chronic_miss) {
        statusCell = `<td style="padding:8px 10px;color:#7f1d1d;font-weight:700;white-space:nowrap;">&#10007; Never documented</td>`;
        rowBg = "#fff1f2";
      } else {
        statusCell = `<td style="padding:8px 10px;color:#b91c1c;font-weight:600;white-space:nowrap;">&#10007; Missed</td>`;
        rowBg = "#fef2f2";
      }
      ...
      return `
        <tr style="background:${rowBg};border-top:1px solid #e5e7eb;">
          <td style="padding:8px 10px;font-weight:600;font-size:13px;">${esc(r.property_name)}${chronicNote}</td>
