...
export function buildProactiveReporting(vendor, allReviews, allTasks) {
  ...
  const complaintReviews = allReviews.filter((r) => {
    ...
    // Require evidence in the text — low rating alone is not sufficient.
    // Guests rate cleanliness low for reasons unrelated to cleaning (old furniture,
    // property condition) without any actionable issue the cleaner could have caught.
    return hasPhysicalComplaint(fullText) || (hasLow && fullText.length > 30);
  });
