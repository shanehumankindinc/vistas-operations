...
import { scanReviewText, hasPositiveKeywords, hasPhysicalComplaint } from "@/lib/keywords";
...
// For each complaint review at a property this vendor cleaned, checks whether
// the vendor filed a Breezeway maintenance task before the guest surfaced the issue.

export function buildProactiveReporting(vendor, allReviews, allTasks) {
  const vendorPropIds = new Set(
    (vendor.enriched_tasks || []).map((t) => t.bz_property_id).filter(Boolean)
  );
  const vendorPropNames = new Set(vendor.properties || []);

  const vendorIndividuals = new Set(
    (vendor.enriched_tasks || [])
      .map((t) => t.individual_name)
      .filter(Boolean)
      .map((n) => n.toLowerCase().trim())
  );
  vendorIndividuals.add(vendor.vendor_name.toLowerCase().trim());

  const cleanTasks = allTasks.filter((t) => isCleanTask(t) && t.scheduled_date);
  const maintTasks = allTasks.filter((t) =>
    (t.task_type || "").toLowerCase().includes("maintenance")
  );

  const complaintReviews = allReviews.filter((r) => {
    const atProp =
      (r.bz_property_id && vendorPropIds.has(r.bz_property_id)) ||
      (r.property_name && vendorPropNames.has(r.property_name));
    if (!atProp) return false;
    const hasLow = r.cleanliness != null && r.cleanliness < 4;
    const fullText = (r.review_text || "") + " " + (r.private_feedback || "");
    return hasLow || hasPhysicalComplaint(fullText);
  });

  const rows = [];

  for (const review of complaintReviews) {
    const reviewDate = review.submitted_at;

    // Most recent clean by this vendor at this property, on or before the review date
    const vendorCleans = cleanTasks
      .filter((t) => {
        const propMatch =
          (t.bz_property_id && review.bz_property_id && t.bz_property_id === review.bz_property_id) ||
          (t.property_name && review.property_name && t.property_name === review.property_name);
        return propMatch && t.vendor_name === vendor.vendor_name && t.scheduled_date <= reviewDate;
      })
      .sort((a, b) => b.scheduled_date.localeCompare(a.scheduled_date));

    if (vendorCleans.length === 0) continue;

    const lastCleanDate = vendorCleans[0].scheduled_date;

    // Was a maintenance task filed by this vendor's team in the window [lastCleanDate, reviewDate]?
    ...
    // Build the complaint excerpt from whichever text field has content
    const complaintText = (review.review_text || "").trim() || (review.private_feedback || "").trim();
    const excerpt = complaintText
      ? complaintText.length > 90
        ? complaintText.slice(0, 89) + "…"
        : complaintText
      : null;

    rows.push({
      property_name: review.property_name || "Unknown property",
  ...
  // Dedupe by property+clean_date: if multiple reviews for same slot, prefer missed (worst case)
  const byKey = {};
...
function renderKpiStrip(vendor) {
  ...
  return `
    <div style="display:flex;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;">
      ...
      ${kpi("TASKS FILED", issues, "via Breezeway", true)}
    </div>`;
}
...
function renderCelebrate(vendor) {
  ...
  // 5-star quotes with property attribution
  const fiveStars = enriched
    .map((t) => ({ review: t.review, property: t.property_name }))
    .filter((x) => x.review?.cleanliness === 5);
  ...
  if (fiveStars.length > 0) {
    ...
    const withText = fiveStars
      .filter((x) => x.review.review_text && hasPositiveKeywords(x.review.review_text))
      .slice(0, 3);

    if (withText.length > 0) {
      const quotes = withText
        .map((x) => `<li style="margin-bottom:8px;">"${esc(x.review.review_text)}"${x.property ? ` <span style="color:#6b7280;font-size:12px;">— ${esc(x.property)}</span>` : ""}</li>`)
        .join("");
  ...
  // Specific task examples (up to 2 with actual descriptions)
  if (issues.length > 0) {
    const notable = issues
    ...
    if (notable.length > 0) {
      ...
      const examples = notable
        .map((i) => {
          const desc = i.description || i.task_title || "issue";
          return `<li style="margin-bottom:6px;">${esc(i.property_name)} (${fmtDate(i.created_at?.slice(0, 10))}): ${esc(desc.slice(0, 100))}</li>`;
        })
        .join("");
      lines.push(`<ul style="margin:0;padding-left:20px;color:#374151;font-size:14px;">${examples}</ul>`);
    } else {
      lines.push(`<p style="margin:10px 0 0;"><strong>You filed ${issues.length} maintenance issue${issues.length !== 1 ? "s" : ""} via Breezeway.</strong> Reporting problems promptly protects the property and the guest experience — this is exactly what Section 3 of your agreement asks for.</p>`);
    }
  }
...
function renderAddress(vendor) {
  ...
  if (lowR.length > 0) {
    const reviewLines = lowR.slice(0, 3).map(({ review, property }) => {
      const txt = review.review_text || review.private_feedback || "";
      const excerpt = txt ? `"${esc(txt.slice(0, 100))}${txt.length > 100 ? "…" : ""}"` : `${review.cleanliness}&#9733; — no written comment`;
      return `<li style="margin-bottom:8px;">${property ? `<strong>${esc(property)}</strong>: ` : ""}${excerpt}</li>`;
    });
    items.push(`
  ...
  // Agreement keyword hits from all reviews (supply, exterior, maintenance)
  const allReviewTexts = enriched
    .flatMap((t) => [t.review?.review_text, t.review?.private_feedback])
    .filter(Boolean);
  const catMap = {};
  for (const txt of allReviewTexts) {
    for (const hit of scanReviewText(txt)) {
      if (!catMap[hit.section]) catMap[hit.section] = { ...hit, count: 0 };
      catMap[hit.section].count++;
    }
  }
  const keywordHits = Object.values(catMap);
  if (keywordHits.length > 0) {
    const hitLines = keywordHits
      .map((h) => `<li style="margin-bottom:6px;">Guests mentioned <strong>${h.matches.slice(0, 3).join(", ")}</strong> — review <strong>Section ${h.section}</strong> of your agreement (${h.label}).</li>`)
      .join("");
    items.push(`
      <div style="margin-bottom:16px;">
  ...
  if (lateCleans.length > 0) {
    const examples = lateCleans.slice(0, 3).map((t) => {
      const finTime = t.finished_cst
        ? `finished ${t.finished_cst.hour}:${String(t.finished_cst.minute).padStart(2, "0")} ${t.tz_abbr || ""}`
    ...
    .map((c) => {
      ...
      return `
        <tr style="border-top:1px solid #e5e7eb;">
          <td style="padding:8px 10px;font-weight:600;font-size:13px;">${esc(c.name)}</td>
...
function renderOneAsk(vendor, proactiveRows) {
  const ask = computeOneAsk(vendor, proactiveRows);
  return `
    <div style="border:1px solid #e5e7eb;border-top:none;background:#f9fafb;padding:22px 28px;">
...
export function buildCleanerReport(vendor, periodStart, periodEnd, { proactiveRows = [], crewBreakdown = null } = {}) {
  ...
  const sections = [
    ...
    renderOneAsk(vendor, proactiveRows),
    renderDisclaimer(),
  ];
