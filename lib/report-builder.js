...
export function assignStatusLabel(score, onTime, cleans, issuesCreated) {
  ...
  const scoreOk = score == null || score >= 4.7;
...
function extractComplaintExcerpt(review) {
  ...
  // No negative signals found — no actionable excerpt to extract
  ...
  return Object.values(crewMap)
    ...
    .map((crew) => {
      ...
      return {
        name: crew.name,
        cleans: crew.cleans,
        review_count: crew.scores.length,
        quality_score:
...
function renderKpiStrip(vendor) {
  ...
  return `
    <div style="display:flex;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;">
      ${kpi("QUALITY SCORE", score, "floor: 4.7 &nbsp;&middot;&nbsp; aim: 5.0")}
...
function renderAddress(vendor, aiSections) {
  ...
  if (lowR.length > 0) {
    const reviewLines = lowR.slice(0, 3).map(({ review, property }) => {
      ...
      const display = excerpt
        ? `"${esc(excerpt)}"`
        : `${review.cleanliness != null ? review.cleanliness + "&#9733;" : "Low score"} — guest didn't share specific feedback. Do a thorough sanity check at this property on your next visit to catch anything that may have been missed.`;
  ...
  // Late cleans — only show if the recorded finish time was actually at or after 4pm,
  // or if no finish time is recorded (overdue). Prevents data inconsistencies from
  // confusingly showing a 10am finish as "late".
  const lateCleans = enriched.filter((t) => {
    if (!t.decided || t.on_time) return false;
    if (t.finished_cst && t.finished_cst.hour < 16) return false;
    return true;
  });
...
function renderProactiveReporting(proactiveRows) {
  ...
  const TIER_LABEL = { 1: "Tier 1", 2: "Tier 2", 3: "Tier 3 — Urgent" };
  const TIER_STYLE = {
  ...
  const TIER_ACTION = {
    1: "Try to clean or fix it before leaving. Create a Breezeway task if it needs more than a quick fix.",
    2: "Try to fix it on the spot. If you can't resolve it, file a Breezeway task before leaving.",
    3: "Do not attempt to fix it. File an urgent Breezeway task and call Guest Services immediately.",
  };
  ...
  const tableRows = proactiveRows
    .map((r) => {
      ...
      const tierChip = r.tier != null
        ? `<div><span style="${TIER_STYLE[r.tier]}">${TIER_LABEL[r.tier]}</span><div style="font-size:11px;color:#6b7280;font-style:italic;margin-top:3px;line-height:1.4;">${TIER_ACTION[r.tier]}</div></div>`
        : "";
      ...
      if (!r.is_complaint) {
        ...
        statusCell = `<td style="padding:8px 10px;color:#065f46;font-weight:600;min-width:160px;">&#10003; Filed${tierChip}</td>`;
        rowBg = "#f0fdf4";
      } else if (r.is_chronic_miss) {
        statusCell = `<td style="padding:8px 10px;color:#7f1d1d;font-weight:700;min-width:160px;">&#10007; Never documented${tierChip}</td>`;
        rowBg = "#fff1f2";
      } else {
        statusCell = `<td style="padding:8px 10px;color:#b91c1c;font-weight:600;min-width:160px;">&#10007; Missed${tierChip}</td>`;
        rowBg = "#fef2f2";
      }

      let scoreDisplay = "—";
      if (r.cleanliness_score != null) {
        const sc = r.cleanliness_score;
        const scoreColor = sc >= 5 ? "#065f46" : sc >= 4.7 ? "#111827" : sc >= 4 ? "#b45309" : "#b91c1c";
        scoreDisplay = `<span style="color:${scoreColor};font-weight:600;">${sc}&#9733;</span> cleanliness`;
      }
...
function renderCrewBreakdown(crewBreakdown, vendor) {
  ...
  const totals = crewBreakdown.reduce(
    (acc, c) => {
      ...
      // Weight quality by review count (not cleans) so the total matches the vendor-level score
      if (c.quality_score != null) { acc.scoreSum += c.quality_score * c.review_count; acc.scoreCount += c.review_count; }
  ...
  const crewRows = crewBreakdown
    .map((c) => {
      const qsVal = c.quality_score != null ? fmt(c.quality_score, 2) : "—";
      const qsColor = c.quality_score != null && c.quality_score < 4.7 ? "color:#b91c1c;font-weight:700;" : "";
      ...
      return `
        <tr style="border-top:1px solid #e5e7eb;">
          ...
          <td style="padding:8px 10px;text-align:center;font-size:13px;${qsColor}">${qsVal}</td>
...
function renderDownloadButton(vendor, periodStart) {
  const d = new Date(periodStart + "T12:00:00Z");
  const monthYear = d.toLocaleDateString("en-US", { month: "short", year: "numeric" }).replace(" ", "-");
  const vendorSlug = vendor.vendor_name.replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-|-$/g, "");
  const filename = `${vendorSlug}_${monthYear}_Performance-Report`;
  return `
    <div class="no-print" style="text-align:right;padding:12px 28px 0;max-width:680px;margin:0 auto;">
      <button id="dl-pdf-btn" onclick="(function(){
        var btn=document.getElementById('dl-pdf-btn');
        ...
        }).from(document.querySelector('.card')).save().then(function(){
          btn.disabled=false;btn.innerHTML='&#x2B73; Download PDF';
        });
      })()" style="padding:7px 16px;background:#0f172a;color:#fff;border:none;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">
        &#x2B73; Download PDF
      </button>
    </div>`;
}
...
export function buildCleanerReport(vendor, periodStart, periodEnd, { proactiveRows = [], crewBreakdown = null, aiSections = null } = {}) {
  ...
  const sections = [
    renderDownloadButton(vendor, periodStart),
