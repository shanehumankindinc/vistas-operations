...
export function buildProactiveReporting(vendor, allReviews, allTasks) {
  ...
  // Negative-sentiment phrases that confirm a cleaning-related complaint when
  // paired with a low cleanliness score. Intentionally excludes "but" (too broad)
  // and generic negativity ("hard", "difficult") that isn't cleaning-specific.
  ...
  const complaintReviews = allReviews.filter((r) => {
    ...
    const fullText = ((r.review_text || "") + " " + (r.private_feedback || "")).trim();
    const textLower = fullText.toLowerCase();
    // Primary signal: physical complaint keywords specific to cleaning/maintenance.
    // Secondary signal: low cleanliness rating + text contains a negative-sentiment phrase.
    // Low rating alone (no supporting text) is dropped — guests can rate cleanliness
    // low for reasons the cleaner couldn't have caught (old furniture, property age).
