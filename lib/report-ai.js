...
export function buildVendorBrief(vendor, market) {
  ...
  // Late cleans — max 3 (only include tasks where finish time was actually at/after 4pm)
  const lateCleans = enriched
    .filter((t) => t.decided && !t.on_time && !(t.finished_cst && t.finished_cst.hour < 16))
    .slice(0, 3)
    .map((t) => ({
      property: t.property_name,
      date: t.scheduled_date,
...
celebrate — 2 to 4 sentences:
  ...
  HARD RULE: Do NOT mention or frame the quality/cleanliness score in celebrate if it is below
  4.7. A score of 3.0 or 4.0 is not an achievement to acknowledge. Celebrating a below-standard
  score is confusing and demoralizing. Instead, focus only on what genuinely went well:
  on-time rate, tasks filed, cleans completed, specific properties worth acknowledging.

address — up to 4 sentences total, or empty string "" if nothing to address:
  The quality score floor is 4.7. The goal is always 5.0. Any score below 4.7 is below
  standard and should be mentioned if the review contains actionable content.
  Write this only if issues exist: late_cleans, short_cleans, complaint_indices non-empty,
  or all_reviews contains cleanliness ratings below 4.7 with actionable complaint text.
  ...
  IMPORTANT: A guest giving 4 stars with positive or neutral text ("great stay", "loved it",
  "minor things") is NOT actionable — do not mention it in address. Only reference a
  low score when the review text describes something the cleaner could have done differently.
  For low-score reviews with actionable content: say "Pointe Royale scored a 3 for
  cleanliness" — never "Review _i 5" or any reference to field names or array positions.
  Property name + score only.
