...
- Output valid JSON with exactly five fields: celebrate, address, one_ask, complaint_indices, complaint_excerpts.
...
complaint_excerpts — object, keys are _i values as strings, values are excerpt strings:
  For every _i in complaint_indices, copy the 1-2 consecutive sentences from that review
  that name the specific physical problem most clearly. Quote the guest's exact words —
  do not paraphrase. Skip generic openers ("unfortunately the unit was not ready") and
  focus on the concrete condition: "The carpet was heavily stained and had a musty/smoky
  odor." or "The router had no power and wi-fi was not working upon check-in."
  Keep each excerpt under 220 characters. Return {} if complaint_indices is empty.
