...
complaint_indices — array of integers (idx values from all_reviews):
...
one_ask — exactly one imperative sentence:
  ...
  Priority: (1) complaint_indices non-empty — file a Breezeway task every time you notice
  an issue before leaving the property; (2) short_cleans — take the full time each
  property needs; (3) late_cleans and on_time_pct < 90 — call the office by 2pm when
  running behind; (4) low cleanliness reviews in all_reviews — address those specific
  details on every clean; (5) tasks_filed = 0 with many cleans — start filing maintenance
  tasks in Breezeway; (6) everything looks strong — name one metric to push even higher.
