...
// Fetch ALL Breezeway properties across all companies — no filtering.
// The cron uses reference_external_property_id to map each property to its Guesty market.
export async function fetchAllBzProperties() {
  const all = [];
  let page = 1;
  const limit = 100;
...
// Legacy: fetch properties for a single market by state code.
// Kept for compatibility but the cron now uses fetchAllBzProperties + Supabase lookup.
export async function fetchBzProperties(market) {
  const { bzState } = MARKETS[market];
  const all = await fetchAllBzProperties();
  return all.filter((p) => p.state === bzState);
}
