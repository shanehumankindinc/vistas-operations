...
// Fetch maintenance/issue tasks for a property (by created_at range, no scheduled_date filter).
// These are tasks created BY cleaners to report issues — they may have no scheduled_date.
export async function fetchBzMaintenanceTasksForProperty(bzPropertyId, propName, fromDate, toDate, market) {
  ...
  while (true) {
    ...
    const rows = Array.isArray(data) ? data : (data.results || data.data || []);
    // Only keep non-cleaning tasks (maintenance, issue, repair, etc.)
    const nonClean = rows.filter((t) => {
      const type = (t.type_department || t.task_type || t.type || "").toLowerCase();
      const name = (t.name || t.task_title || t.title || "").toLowerCase();
      return type.includes("maintenance") || type.includes("issue") || type.includes("repair") ||
             name.includes("maintenance") || name.includes("issue") || name.includes("repair") ||
             name.includes("damage") || name.includes("broken");
    });
    results.push(...nonClean.map((t) => ({ ...t, _propName: propName, _bzId: bzPropertyId })));
    if (rows.length < limit) break;
    page++;
  }
  return results;
